...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        guessButton.setOnClickListener {
            ...
            val userInput = findViewById<EditText>(R.id.userTextInput)

            if (guesses == 0) {
                val guess1 = findViewById<TextView>(R.id.userGuess1answerinput)
                val guessCheck1 = findViewById<TextView>(R.id.guessCheck1AnsDisplay)
                guess1.text = userInput.text.toString()

                val checkAns1 = guess1.text.toString().uppercase()
                val guessAnswer1 = checkGuess(checkAns1)
                guessCheck1.text = guessAnswer1
                guesses++

                if (guessCheck1.text == "OOOO") {
                ...
                val guess2 = findViewById<TextView>(R.id.userGuess2answerinput)
                val guessCheck2 = findViewById<TextView>(R.id.guessCheck2AnsDisplay)
                guess2.text = userInput.text.toString()

                val checkAns2 = guess2.text.toString().uppercase()
                val guessAnswer2 = checkGuess(checkAns2)
                guessCheck2.text = guessAnswer2
                guesses++

                if (guessCheck2.text == "OOOO") {
                    Toast.makeText(this, "$seek", Toast.LENGTH_LONG).show()
                    answerDisplay.visibility = View.VISIBLE
                    textView.text = findViewById<TextView>(R.id.answer).toString()
                }
            } else if (guesses == 2) {
                val guess3 = findViewById<TextView>(R.id.userGuess3AnswerInput)
                val guessCheck3 = findViewById<TextView>(R.id.guessCheck3AnsDisplay)
                guess3.text = userInput.text.toString()

                val checkAns3 = guess3.text.toString().uppercase()
                val guessAnswer3 = checkGuess(checkAns3)
                guessCheck3.text = guessAnswer3
                guesses++

                if (guessCheck3.text == "OOOO") {
                    Toast.makeText(this, "$seek", Toast.LENGTH_LONG).show()
                    answerDisplay.visibility = View.VISIBLE
                    textView.text = findViewById<TextView>(R.id.answer).toString()
    ...
    object FourLetterWordList {
        ...
        val fourLetterWords = "Area,Army,Baby,Back,Ball,Band,Bank,Base,Bill,Body,Book,Call,Card,Care,Case,Cash,City,Club," +
